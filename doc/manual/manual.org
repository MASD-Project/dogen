...
8. The =projects= folder contains the Dogen source code, in any of the
   ...
   cohesive unit work - a component. For example, a set of ruby
   scripts around a given theme (sanity) are a project. We freely mix
   projects which exist solely to test dogen with projects that
