...
Before we go into Dogen usage proper, we will start with a quick tour
of the command line options available. Before we start we should make
sure Dogen is operational. Run:

: $ dogen_driver --version

If you are running it from the build directory and on UNIX, you may
need to refer to the current directory:

: $ ./dogen_driver --version

If this is the case, henceforth you will have to add =./= to all of
...
Now we have confirmed Dogen is operational, lets have a look at all
...
which attempt to aggregate related functionality. These are as
follows:

- *General options*: these are not directly related to operational
  aspects, but provide information about the application. We have
  already seen both =help= and =version=, the most important of the
...
With this we can now perform our first code generation.

*** Hello World in Dogen

In order to generate some code you need only one command line option:
=--target=. It is part of the modeling group, and specifies the file
containing the model one wishes to code generate. The target file is a
UML Dia diagram, crafted according to the rules stipulated by
Dogen.

There are a couple of preparatory steps required before we can start
using Dogen. Place yourself in a new directory of your choosing, and
copy across the Hello World model from our git tree into this
directory:
...
Notice that the diagram's file name does not contain any spaces. This
is important because it will be used as the name of the project, and
the top-level namespace. 

You should also see a directory called =log=
where the log file is stored: =dogen.log=. By default Dogen is not
particularly expressive, so there won't be much on the log file to
look at.

In addition to this, there should also be a directory called
=hello_world=. To understand the meaning and the rationale of the
directory structure you should read (FIXME add link) for details. For
now we will just open 
