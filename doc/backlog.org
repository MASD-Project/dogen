...
*** Forward declaration of enumerations is incorrect                  :story:
...
*** Move injection of system types into SML                           :story:
...
*** Improve exception reporting in tests                              :story:
...
**** Add support for multiple stereotypes to Dia
...
**** Add back-end specific properties to models

We need a way to attach facets to pods, or perhaps we need some
concept at the SML level which maps to the facet in a language
agnostic way. This is not very good since its obvious some back-ends
will not support these concepts (e.g. what does streaming mean to
SQL?). The better alternative is to have some kind of back-end
specific properties in SML.
...
*** Generate the Dia model from dogen
