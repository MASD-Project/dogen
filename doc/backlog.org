...
- create registrar for each model which registers types
- create model dependencies, automatically populated
- use model dependencies to load dependent models and to error if
  there are missing references
- add registrar depdendencies based on model dependencies
- ensure we ignore all system models
- remove config from source control, move it back to output directory
