...
- entity must have at least one key.
...
- pod marked as is comparable must only have properties with is
  comparable types.
...
- remove view model namespace and postfix. These are just types at the
  ...
  where the name is a C++ keyword (e.g. =class=, =namespace=), create
  a well known prefix like eCore. Question is what prefix to use.
- clean up the SML to C++ transformer: split graph from transformer,
  split visitor from transformer, etc.
