...
#+CAPTION: Clock summary at [2017-02-26 Sun 00:31]
| <75>                                                                        |         |       |       |       |
| Headline                                                                    | Time    |       |       |     % |
|-----------------------------------------------------------------------------+---------+-------+-------+-------|
| *Total time*                                                                | *20:58* |       |       | 100.0 |
|-----------------------------------------------------------------------------+---------+-------+-------+-------|
| Stories                                                                     | 20:58   |       |       | 100.0 |
| Active                                                                      |         | 20:58 |       | 100.0 |
| STARTED Sprint and product backlog grooming                                 |         |       |  0:24 |   1.9 |
| COMPLETED Edit release notes for previous sprint                            |         |       |  0:30 |   2.4 |
| COMPLETED Fix the handling of ODB pragmas for yarn primitives               |         |       |  0:56 |   4.5 |
| COMPLETED Rename intermediate model expander and model factory              |         |       |  0:53 |   4.2 |
| STARTED Injection: do not split out root annotation from intermediate model |         |       |  0:26 |   2.1 |
| STARTED ODB Fixes to support Oracle                                         |         |       |  1:12 |   5.7 |
| STARTED Implement the database model using Northwind                        |         |       | 16:37 |  79.3 |
...
*** STARTED Injection: do not split out root annotation from intermediate model :story:
...
*** STARTED ODB Fixes to support Oracle                               :story:
...
*** Make ODB backend settable from meta-data                          :story:
...
*** Clean up odb implementation                                       :story:
...
related to ORM (object-relational mapping). These are:
...
- yarn.orm.database: container of supported database types. Create
  enumeration.
- yarn.orm.override: if true, orm meta-data is ignored altogether. User is
  expected to manually supply ODB pragmas.
- yarn.orm.schema_name: model module level or element
- yarn.orm.table_name: element
- yarn.orm.is_primary_key: attribute
- yarn.orm.column_name: attribute
- yarn.orm.nullable: attribute
- yarn.org.naming_style: capitals, lowercase, as_is.
