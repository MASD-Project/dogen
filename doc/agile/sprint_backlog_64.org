...
Clock summary at [2015-04-26 Sun 08:33]
...
| STARTED Create a forward declarations formatter using stitch    |         |       | 0:53 |
#+end:
...
- solve the problem of relocatable formatters. Basically we want to be
  able to use forward declarations from within any facet. At present
  that is not possible because formatters have hard-coded ownership
  hierarchies. In addition, there is the potential for inclusion
  dependencies generation which is also facet specific. However in the
  case of forward declarations this is not an issue. It seems that for
  forward declarations we could get away with supplying the facet when
  constructing the formatter and then having it registered against
  every facet. Investigate this possibility.
