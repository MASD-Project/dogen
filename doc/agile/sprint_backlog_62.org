...
Clock summary at [2015-03-26 Thu 15:27]
...
| STARTED Improve performance in populating file path                      |         |       | 1:57 |
#+end:
...
*** STARTED Improve performance in populating file path               :story:
...
- only populate file path if the type is in the target model. only
  populate include path if inclusion_required is true.

*** Consider creating field definition templates                      :story:
