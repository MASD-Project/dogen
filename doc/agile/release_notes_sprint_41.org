...
New understanding: the problem appears to be with boost 1.54;
...
seems like the easiest thing is to require boost 1.55, but
unfortunately a lot of distributions are currently shipping with 1.54;
in addition, there are no debs for 1.55 yet so we can't even tell if
the changes to build without PFH have worked.
