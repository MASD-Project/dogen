...
Clock summary at [2014-01-08 Wed 00:02]

| Headline                                                                | Time    |       |       |
|-------------------------------------------------------------------------+---------+-------+-------|
| *Total time*                                                            | *38:03* |       |       |
|-------------------------------------------------------------------------+---------+-------+-------|
| Development Stories Implemented                                         |         | 38:03 |       |
...
| ANALYSIS Design a cleaner meta-data API                                 |         |       |  0:23 |
| IMPLEMENTATION Update all tests to use =om= formatter                   |         |       |  8:09 |
...
additional set of properties which are formatter specific. This
vaguely resembles the decorator pattern. We have tried in the past to
create an entire output model, totally divorced from SML, and found
out that it had far too much in common with SML to justify its
existence. At the same time, creating a set of classes to augment SML
in OM without adding properties directly to SML seems a bit strange -
we will end up having to pass the OM class plus the SML class on every
method. This requires a bit more thinking.
