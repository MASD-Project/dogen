...
#+CAPTION: Clock summary at [2016-09-21 Wed 16:31]
| <75>                                                                        |         |       |       |       |
| Headline                                                                    | Time    |       |       |     % |
|-----------------------------------------------------------------------------+---------+-------+-------+-------|
| *Total time*                                                                | *58:19* |       |       | 100.0 |
|-----------------------------------------------------------------------------+---------+-------+-------+-------|
| Stories                                                                     | 58:19   |       |       | 100.0 |
| Active                                                                      |         | 58:19 |       | 100.0 |
| STARTED Sprint and product backlog grooming                                 |         |       |  3:33 |   6.1 |
| COMPLETED Fix chocolatey warning                                            |         |       |  0:08 |   0.2 |
| COMPLETED Edit release notes for previous sprint                            |         |       |  0:27 |   0.8 |
| COMPLETED Analysis on current state of file path generation                 |         |       |  0:57 |   1.6 |
| CANCELLED Remove inclusion dependencies builder factory                     |         |       |  0:14 |   0.4 |
| COMPLETED Analysis on formatters, elements and extensibility                |         |       |  1:18 |   2.2 |
| COMPLETED Refactor =inclusion_dependencies_provider_interface=              |         |       |  1:12 |   2.1 |
| COMPLETED Investigate broken windows builds                                 |         |       |  0:32 |   0.9 |
| COMPLETED Refactor path derivatives factory for usage in formatters         |         |       |  5:14 |   9.0 |
| COMPLETED Move file path generation to formatters                           |         |       |  1:58 |   3.4 |
| CANCELLED Path derivatives factory generates all possible includes          |         |       |  3:07 |   5.3 |
| COMPLETED Analysis on inclusion derivatives and path derivatives            |         |       |  1:04 |   1.8 |
| COMPLETED Split path factory from provider factory                          |         |       |  2:32 |   4.3 |
| COMPLETED Use providers when building inclusion directives                  |         |       | 12:01 |  20.6 |
| COMPLETED Fix AppVeyor notifications                                        |         |       |  0:22 |   0.6 |
| COMPLETED Static use of element types does more harm than good              |         |       |  0:32 |   0.9 |
| COMPLETED Analysis: major rethink of the implementation of quilt            |         |       |  1:30 |   2.6 |
| CANCELLED Create a simple Emacs mode for stitch                             |         |       |  1:12 |   2.1 |
| COMPLETED Add facet folder and formatter enablement to formatter properties |         |       |  2:52 |   4.9 |
| COMPLETED Move =cmakelists_info= to fabric                                  |         |       |  6:34 |  11.3 |
| COMPLETED Move =odb_options_info= to fabric                                 |         |       |  1:29 |   2.5 |
| COMPLETED Remove =formattable_origin_type=                                  |         |       |  0:17 |   0.5 |
| COMPLETED Remove formattable origin type                                    |         |       |  2:25 |   4.1 |
| COMPLETED Rename config model to options                                    |         |       |  1:07 |   1.9 |
| COMPLETED Rename annotation to decoration in formatters model               |         |       |  0:30 |   0.9 |
| COMPLETED Fix appveyor Visual Studio build                                  |         |       |  1:04 |   1.8 |
| COMPLETED Big rename for "final" terminology clean up                       |         |       |  2:19 |   4.0 |
| STARTED Tidy-up file configuration in formatters                            |         |       |  0:56 |   1.6 |
| STARTED Add type index to name and to formatter interface                   |         |       |  0:53 |   1.5 |
...
*** STARTED Tidy-up file configuration in formatters                  :story:
    CLOCK: [2016-09-21 Wed 15:35]--[2016-09-21 Wed 16:31] =>  0:56

We never did like the "file" prefix, since many things can be
construed as "file configuration". In a certain way, all configuration
is ultimately file configuration since it affects the generation of
files. However, things became clearer with the decoration rename:
this is the decoration configuration.
...
*** Formatter properties are not optional in assistant                :story:
