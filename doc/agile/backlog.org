...
*** Add =try_get_key= to reader                                       :story:

At present we are doing a lot of =has_key= calls followed by =get=
using the meta-data reader in SML. We should have a single call that
does both, returning perhaps a pair of bool, value.
...
*** It should be possible to add non-licensed files to om             :story:
...
*** Use clang to generate system models                               :story:
...
*** Create the notion of toggle-able features                         :story:
