...
*** Referenced models do not have link time dependencies              :story:

For some reason, when a model references another model it doesn't seem
to result in a link time dependency. To solve this problem we end up
having to disable makefiles and then add dependencies manually.

In addition, we also need a way to supply user defined linked
libraries, which could be CMake variables. This could be done via
implementation specific parameters at the model level.
...
*** Automatic commenting of enumeration properties

When a pod has a property of an enumeration type, there is not much
point in manually commenting the property. We could just use the brief
of the enumeration if the user didn't supply any documentation.
