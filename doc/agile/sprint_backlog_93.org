...
#+CAPTION: Clock summary at [2016-12-05 Mon 00:34]
| <75>                                                                        |         |       |      |       |
| Headline                                                                    | Time    |       |      |     % |
|-----------------------------------------------------------------------------+---------+-------+------+-------|
| *Total time*                                                                | *16:56* |       |      | 100.0 |
|-----------------------------------------------------------------------------+---------+-------+------+-------|
| Stories                                                                     | 16:56   |       |      | 100.0 |
| Active                                                                      |         | 16:56 |      | 100.0 |
| STARTED Sprint and product backlog grooming                                 |         |       | 1:51 |  10.9 |
| COMPLETED Edit release notes for previous sprint                            |         |       | 1:02 |   6.1 |
| COMPLETED Update readme                                                     |         |       | 0:11 |   1.1 |
| COMPLETED Test data generation for dates generates invalid dates            |         |       | 0:33 |   3.2 |
| COMPLETED Handle missing stitch templates scenario                          |         |       | 1:04 |   6.3 |
| COMPLETED Move project directory to output options                          |         |       | 1:10 |   6.9 |
| STARTED Update the conceptual model to clarify backends and kernels         |         |       | 3:16 |  19.3 |
| STARTED Clean up kernel and backend confusion                               |         |       | 0:54 |   5.3 |
| STARTED Add support for Decimal numbers                                     |         |       | 0:32 |   3.1 |
| STARTED Add support for C#                                                  |         |       | 5:46 |  34.1 |
| STARTED Add a "language" directory for each backend                         |         |       | 0:37 |   3.6 |
...
*** STARTED Update the conceptual model to clarify backends and kernels :story:
...
*** STARTED Clean up kernel and backend confusion                     :story:
...
- rename model in annotations to backend (templates).
- add backend to archetype location, and update existing kernel to be
  just the kernel rather than the backend.
...
- add quilt level fields. Ensure we have at least one formatter in
  =quilt.csharp=, with a archetype location so that we generate its
  fields.
