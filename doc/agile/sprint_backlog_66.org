...
Clock summary at [2015-05-17 Sun 19:52]
...
| STARTED Implement types class header formatter using stitch                 |         |       | 2:43 |
#+end:
...
- we can only determine if a formatter is enabled or a facet is
  ...
  the current qname every time we want to ask a question.
