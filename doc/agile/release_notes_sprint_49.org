...
Clock summary at [2014-05-13 Tue 08:37]

| Headline                                                        | Time    |       |      |
|-----------------------------------------------------------------+---------+-------+------|
| *Total time*                                                    | *19:16* |       |      |
|-----------------------------------------------------------------+---------+-------+------|
| Development Stories Implemented                                 |         | 19:16 |      |
...
| IMPLEMENTATION How to supply concrete entities to formatters    |         |       | 0:27 |
#+end:
...
unrepresentable - e.g. one can pass a concrete class to the incorrect
formatter and the type system won't help us detect this.
...
- do not use composition of =source_file= and =entity=. We could
  ...
  the formatters. The downside is that the contents of the file
  (header guards, file names, etc) only make sense for that entity in
  question. Again we create an opportunity for invalid states (entity
  with wrong file). Also, the project class will became a bit messy.
