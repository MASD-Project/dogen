...
#+CAPTION: Clock summary at [2019-11-20 Wed 19:47]
| <75>                                   |        |      |      |       |
| Headline                               | Time   |      |      |     % |
|----------------------------------------+--------+------+------+-------|
| *Total time*                           | *3:42* |      |      | 100.0 |
|----------------------------------------+--------+------+------+-------|
| Stories                                | 3:42   |      |      | 100.0 |
| Active                                 |        | 3:42 |      | 100.0 |
| Edit release notes for previous sprint |        |      | 3:11 |  86.0 |
| Sprint and product backlog grooming    |        |      | 0:31 |  14.0 |
...
#+begin_src markdown
**DRAFT: Release notes under construction. please ignore them for now**
...
The sprint's key feature is _variability overrides_. It was specifically designed to allow the overriding of model profiles, so that the same diagram can produce different "kinds" of outputs. In order to understand how the feature came about, we need to revisit a bit of Dogen history. As you may recall, amongst many other things, profiles control which facets are to be expressed for a given modeling element (e.g. Boost serialisation, hashing, and so forth). In the MDE domain, these ideas have been generalised into "variability modeling", because, taken as a whole, they give you a dimension in which you can "vary" how any given modeling element is expressed - i.e. entities can output different source code depending on their "configuration". Dogen has long made use of this feature, enabling certain model elements to support say hashing whereas others can support say lexical casting.

![Dogen's profile model](https://github.com/MASD-Project/dogen/raw/master/doc/blog/images/profiles_model.png)

Of course, models are associated to profiles by using nothing but meta-data, which ultimately is how all variability information is transported. A typical Dogen model looks like so:

```
#DOGEN masd.variability.profile=dogen.profiles.base.default_profile
```

This then results in reusing the configuration defined by ```default_profile``` - an entitty in the referenced model
```dogen.profiles```.

Profiles have evolved from the observation that the meta-data used for configuration is, in many cases, common to several models and therefore should be shared. This approach has served us well thus far, but one implicit assumption it carried is that there would always only be a need for /one profile per model/. In truth, we have two
distinct - and incompatible - needs when generating Dogen's own models:

- from a production perspective, we want to generate the smallest amount of code required so that we avoid bloating our binaries with unecessary code. Thus we want our profiles to be lean and mean.
- from a development and Q&A perspective, we want to test all possible facets with realistic use cases so that we can validate empirically the quality of the generated code. Dogen's own models are a great sample point for this validation, and should therefore make use of as many facets as possible.

This problem was not obvious initially because we could afford to generate all facets for all models and ignore the bloat. However, as the number of facets increased and as the number of elements in each model grew, we eventually ran out of build time to compile all of the generated code. Since then, we have resorted to "clever" hacks to allow these two conflicting requirements to coexist, such as enabling only a few facets in certain models. In truth, the low hanging fruit had been exhausted a few sprints ago, and we consistently started to have build failures due to time outs.

Not being able to trust your CI is a showstopper, of course, so this sprint we sat down to solve this problem in a somewhat general manner. Variability overrides was the chosen solution, and it was implemented as follows.

A new command line option was added to the Processing section for this feature, called ```--variability-override ```. The option expects a tuple of up to five elements:
