...
#+CAPTION: Clock summary at [2017-07-14 Fri 20:33]
| <75>                                                                        |         |       |      |       |
| Headline                                                                    | Time    |       |      |     % |
|-----------------------------------------------------------------------------+---------+-------+------+-------|
| *Total time*                                                                | *41:32* |       |      | 100.0 |
|-----------------------------------------------------------------------------+---------+-------+------+-------|
| Stories                                                                     | 41:32   |       |      | 100.0 |
| Active                                                                      |         | 41:32 |      | 100.0 |
| STARTED Sprint and product backlog grooming                                 |         |       | 1:07 |   2.7 |
| COMPLETED Edit release notes for previous sprint                            |         |       | 0:24 |   1.0 |
| COMPLETED Implement the context class                                       |         |       | 1:12 |   2.9 |
| COMPLETED Implement the exogenous transformation chain                      |         |       | 4:11 |  10.1 |
| COMPLETED Implement the pre-processing chain                                |         |       | 1:35 |   3.8 |
| COMPLETED Refactor mapping repository factory                               |         |       | 0:12 |   0.5 |
| COMPLETED Implement the output languages transform                          |         |       | 0:37 |   1.5 |
| COMPLETED Use forward declaration for context                               |         |       | 0:20 |   0.8 |
| COMPLETED Implement the references chain                                    |         |       | 2:43 |   6.5 |
| COMPLETED Implement the merge transform                                     |         |       | 0:40 |   1.6 |
| COMPLETED Implement the model generation chain                              |         |       | 0:13 |   0.5 |
| COMPLETED Implement the model assembly chain                                |         |       | 0:50 |   2.0 |
| COMPLETED Implement the post-processing chain                               |         |       | 3:31 |   8.5 |
| COMPLETED Move helpers from =transforms= namespace                          |         |       | 0:45 |   1.8 |
| COMPLETED Replace expanders with transforms                                 |         |       | 0:03 |   0.1 |
| COMPLETED Port all expander unit tests to transforms                        |         |       | 0:42 |   1.7 |
| COMPLETED Remove all legacy classes from yarn                               |         |       | 0:55 |   2.2 |
| COMPLETED Implement the mapper as a transform                               |         |       | 0:46 |   1.8 |
| COMPLETED Move helpers from main yarn namespace                             |         |       | 1:29 |   3.6 |
| COMPLETED Move all data types into its own namespace                        |         |       | 9:00 |  21.7 |
| CANCELLED Move artefacts from formatters into meta-model                    |         |       | 0:54 |   2.2 |
| CANCELLED Use transformation context in quilt                               |         |       | 0:20 |   0.8 |
| COMPLETED Create an options validator                                       |         |       | 0:39 |   1.6 |
| COMPLETED Implement the code-generation chain                               |         |       | 4:33 |  11.0 |
| COMPLETED Fix options test break on windows                                 |         |       | 0:10 |   0.4 |
| STARTED Investigate why gcc builds are slower than clang                    |         |       | 0:41 |   1.6 |
| STARTED Move enablement into yarn                                           |         |       | 3:00 |   7.2 |
...
*** STARTED Investigate why gcc builds are slower than clang          :story:
