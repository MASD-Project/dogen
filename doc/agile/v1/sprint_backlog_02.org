...
#+CAPTION: Clock summary at [2017-07-12 Wed 21:13]
| <75>                                                                        |         |       |      |       |
| Headline                                                                    | Time    |       |      |     % |
|-----------------------------------------------------------------------------+---------+-------+------+-------|
| *Total time*                                                                | *31:50* |       |      | 100.0 |
|-----------------------------------------------------------------------------+---------+-------+------+-------|
| Stories                                                                     | 31:50   |       |      | 100.0 |
| Active                                                                      |         | 31:50 |      | 100.0 |
| STARTED Sprint and product backlog grooming                                 |         |       | 0:48 |   2.5 |
| COMPLETED Edit release notes for previous sprint                            |         |       | 0:24 |   1.3 |
| COMPLETED Implement the context class                                       |         |       | 1:12 |   3.8 |
| COMPLETED Implement the exogenous transformation chain                      |         |       | 4:11 |  13.1 |
| COMPLETED Implement the pre-processing chain                                |         |       | 1:35 |   5.0 |
| COMPLETED Refactor mapping repository factory                               |         |       | 0:12 |   0.6 |
| COMPLETED Implement the output languages transform                          |         |       | 0:37 |   1.9 |
| COMPLETED Use forward declaration for context                               |         |       | 0:20 |   1.0 |
| COMPLETED Implement the references chain                                    |         |       | 2:43 |   8.5 |
| COMPLETED Implement the merge transform                                     |         |       | 0:40 |   2.1 |
| COMPLETED Implement the model generation chain                              |         |       | 0:13 |   0.7 |
| COMPLETED Implement the model assembly chain                                |         |       | 0:50 |   2.6 |
| COMPLETED Implement the post-processing chain                               |         |       | 3:31 |  11.0 |
| COMPLETED Move helpers from =transforms= namespace                          |         |       | 0:45 |   2.4 |
| COMPLETED Replace expanders with transforms                                 |         |       | 0:03 |   0.2 |
| COMPLETED Port all expander unit tests to transforms                        |         |       | 0:42 |   2.2 |
| COMPLETED Remove all legacy classes from yarn                               |         |       | 0:55 |   2.9 |
| COMPLETED Implement the mapper as a transform                               |         |       | 0:46 |   2.4 |
| COMPLETED Move helpers from main yarn namespace                             |         |       | 1:29 |   4.7 |
| COMPLETED Move all data types into its own namespace                        |         |       | 9:00 |  28.3 |
| CANCELLED Move artefacts from formatters into meta-model                    |         |       | 0:54 |   2.8 |
