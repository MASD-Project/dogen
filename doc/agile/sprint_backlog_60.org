...
Clock summary at [2015-01-28 Wed 23:12]
...
| STARTED Remove optionality from =obtain_field_definition=             |         |       | 2:10 |
...
- actually, thus approach won't work for SML - we need to pass in the
  flag via the workflow, into the transformer. For that amount of
  work, its easier to solve the dynamic workflow re-creation story and
  just pass it in rather than the flag.
