...
#+CAPTION: Clock summary at [2017-01-26 Thu 14:32]
| <75>                                                                        |         |       |       |       |
| Headline                                                                    | Time    |       |       |     % |
|-----------------------------------------------------------------------------+---------+-------+-------+-------|
| *Total time*                                                                | *64:28* |       |       | 100.0 |
|-----------------------------------------------------------------------------+---------+-------+-------+-------|
| Stories                                                                     | 64:28   |       |       | 100.0 |
| Active                                                                      |         | 64:28 |       | 100.0 |
| STARTED Sprint and product backlog grooming                                 |         |       |  2:50 |   4.4 |
| COMPLETED Edit release notes for previous sprint                            |         |       |  0:35 |   0.9 |
| COMPLETED Add test in knit to validate upsilon                              |         |       |  0:51 |   1.3 |
| COMPLETED Create an upsilon yarn frontend                                   |         |       | 10:18 |  16.0 |
| COMPLETED Fields with collections in upsilon cause errors                   |         |       |  1:20 |   2.1 |
| COMPLETED Bind language to kernel                                           |         |       |  1:29 |   2.3 |
| COMPLETED Upsilon can have collections of collections                       |         |       |  1:01 |   1.6 |
| COMPLETED Upsilon test borked due to no output                              |         |       |  0:21 |   0.5 |
| COMPLETED Add multi-language support                                        |         |       | 13:28 |  20.9 |
| COMPLETED Add a type mapper                                                 |         |       |  6:19 |   9.8 |
| COMPLETED Backslashes in strings cause JSON parsing to fail                 |         |       |  0:49 |   1.3 |
| COMPLETED Add log-level to command line                                     |         |       |  1:20 |   2.1 |
| COMPLETED Fix recursive path lookup for upsilon                             |         |       |  1:02 |   1.6 |
| COMPLETED In JSON hydrator rename simple name to simple                     |         |       |  1:03 |   1.6 |
| COMPLETED Add tailor support for upsilon                                    |         |       |  0:16 |   0.4 |
| COMPLETED Add support for pointers to mapper in upsilon                     |         |       |  4:44 |   7.3 |
| COMPLETED Discuss the current approach with end users                       |         |       |  1:31 |   2.4 |
| STARTED Rename primitives to built-insert                                   |         |       |  2:11 |   3.4 |
| STARTED Add mapping support between upsilon and LAM                         |         |       |  1:56 |   3.0 |
| STARTED Make the Zeta model compilable                                      |         |       |  0:45 |   1.2 |
| STARTED Add support for Language Agnostic Models (LAM)                      |         |       |  6:55 |  10.7 |
| STARTED Map upsilon primitives to intrinsics                                |         |       |  3:04 |   4.8 |
| STARTED Strong type aliasing                                                |         |       |  0:20 |   0.5 |
...
- add a new yarn element: primitive.
- add a stereotype for primitive.
- add a meta-data parameter for the underlying type. Make it the same
  as for enumerations. Read the meta-data parameter somewhere in yarn
  expansion.
- add a

- Add validation to ensure the element is always a built-in. Actually,
  this is fine for enumerations but not for "primitives". We need an
  additional parameter on each element (can be the underlying element
  of a primitive?).
- add formatters.
