...
Clock summary at [2014-11-03 Mon 19:24]

| Headline                                           | Time   |      |      |
|----------------------------------------------------+--------+------+------|
| *Total time*                                       | *1:38* |      |      |
|----------------------------------------------------+--------+------+------|
| Development Stories Implemented                    |        | 1:38 |      |
| IMPLEMENTATION Sprint and product backlog grooming |        |      | 0:16 |
| COMPLETED Sort out postgres update problems        |        |      | 0:36 |
| COMPLETED Refactor static data initialisation      |        |      | 0:43 |
...
- it is a shame that we interface with the registrar via the workflow
  though. However it is not worth spending too much time on this for
  now.
