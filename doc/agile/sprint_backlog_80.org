...
#+CAPTION: Clock summary at [2016-02-09 Tue 23:19]
| <75>                                                                        |         |       |      |       |
| Headline                                                                    | Time    |       |      |     % |
|-----------------------------------------------------------------------------+---------+-------+------+-------|
| *Total time*                                                                | *15:33* |       |      | 100.0 |
|-----------------------------------------------------------------------------+---------+-------+------+-------|
| Stories                                                                     | 15:33   |       |      | 100.0 |
| Active                                                                      |         | 15:33 |      | 100.0 |
| STARTED Sprint and product backlog grooming                                 |         |       | 3:21 |  21.5 |
| COMPLETED Make context non-generatable                                      |         |       | 0:49 |   5.3 |
| COMPLETED Setup logging syntax highlighting on emacs                        |         |       | 2:32 |  16.3 |
| COMPLETED Create next interesting blog post                                 |         |       | 0:37 |   4.0 |
| STARTED Help conan with strange warning                                     |         |       | 0:51 |   5.5 |
| STARTED Add an helper method interface                                      |         |       | 6:56 |  44.6 |
| STARTED Rename =yarn::nested_name= to =yarn::name_tree=                     |         |       | 0:27 |   2.9 |
...
*** STARTED Rename =yarn::nested_name= to =yarn::name_tree=           :story:
...
*** Add index support for =yarn::nested_name=                         :story:

Add an index to =yarn::nested_name= and add a heigth too. Compute it
as part of the existing yarn infrastructure - nested name builder is
the best place. The index starts at zero for the root nested name and
increments by one as we recurse. We can go for depth first but it
doesn't really matter what approach we use for naming provided the
names are unique (for a given property) and go from 0 to the height of
the tree.
...
*** Add properties for =yarn::nested_name=                            :story:
