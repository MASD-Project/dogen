...
we could express two things: the "key" that we want to
  "compute" and the "kind" of computation we want to perform, we could
  probably come up with a generic algorithm
