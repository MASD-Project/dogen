...
#include "dogen.modeling/hash/meta_model/model_hash.hpp"
...
inline std::size_t hash_std_unordered_set_dogen_modeling_meta_model_element_archetype(const std::unordered_set<dogen::modeling::meta_model::element_archetype>& v) {
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i);
    }
...
std::size_t model_hasher::hash(const model& v) {
    ...
    combine(seed, hash_std_unordered_set_dogen_modeling_meta_model_element_archetype(v.enabled_archetype_for_element()));
