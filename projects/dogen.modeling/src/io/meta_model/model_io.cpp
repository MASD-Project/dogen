...
inline std::ostream& operator<<(std::ostream& s, const std::unordered_set<dogen::modeling::meta_model::element_archetype>& v) {
    s << "[ ";
    for (auto i(v.begin()); i != v.end(); ++i) {
        if (i != v.begin()) s << ", ";
        s << *i;
...
std::ostream& operator<<(std::ostream& s, const model& v) {
    ...
    s << " { "
      << "\"__type__\": " << "\"dogen::modeling::meta_model::model\"" << ", "
      << "\"name\": " << v.name() << ", "
