...
create_dogen_modeling_meta_model_name_tree(const unsigned int) {
    return dogen::modeling::meta_model::name_tree();
}

std::list<dogen::modeling::meta_model::name_tree> create_std_list_dogen_modeling_meta_model_name_tree(unsigned int position) {
    std::list<dogen::modeling::meta_model::name_tree> r;
    for (unsigned int i(0); i < 4; ++i) {
        r.push_back(create_dogen_modeling_meta_model_name_tree(position + i));
    }
    return r;
}
...
populate(const unsigned int position, result_type& v) {
    v.current(create_dogen_modeling_meta_model_name(position + 0));
    v.children(create_std_list_dogen_modeling_meta_model_name_tree(position + 1));
    v.are_children_opaque(create_bool(position + 2));
    v.is_circular_dependency(create_bool(position + 3));
    v.qualified(create_std_map_dogen_modeling_meta_model_languages_std_string(position + 4));
