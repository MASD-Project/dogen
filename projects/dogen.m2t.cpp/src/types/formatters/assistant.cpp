...
#include "dogen.m2t.cpp/types/formatters/lexical_cast/traits.hpp"
#include "dogen.m2t.cpp/types/formatters/odb/traits.hpp"
#include "dogen.m2t.cpp/types/formatters/hash/traits.hpp"
#include "dogen.m2t.cpp/types/formatters/tests/traits.hpp"
...
bool assistant::is_archetype_enabled(const std::string& archetype) const {
    m2t::entities::element_archetype
...
void assistant::comment(const std::string& c) {
    ...
    m2t::formatters::comment_formatter f(
        ...
        m2t::formatters::comment_style::c_style,
        !last_line_is_blank);
    f.format(stream(), c);
}
...
comment_start_method_group(const std::string& documentation,
    ...
    {
        m2t::formatters::positive_indenter_scope pis(stream());
        m2t::formatters::comment_formatter f(
            !start_on_first_line,
            use_documentation_tool_markup,
            !documenting_previous_identifier,
...
std::string assistant::comment_inline(const std::string& c) const {
    ...
    m2t::formatters::comment_formatter f(
        ...
        use_documentation_tool_markup,
        documenting_previous_identifier,
        m2t::formatters::comment_style::cpp_style,
        !last_line_is_blank);
...
names_with_enabled_archetype(const std::string& archetype,
    ...
    for (const auto& n : names) {
        ...
        m2t::entities::element_archetype ea(id, archetype);
        const auto& eafe(context_.enabled_archetype_for_element());
        const auto i(eafe.find(ea));
        const bool is_disabled(i == eafe.end());
        if (is_disabled)
...
physical::entities::artefact assistant::make_artefact() const {
    ...
    r.logical_location().qualified_name(element_.name().qualified().dot());
