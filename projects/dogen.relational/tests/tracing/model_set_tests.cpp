...
#include "dogen.relational/io/tracing/model_set_io.hpp"
#include "dogen.relational/types/tracing/model_set.hpp"
#include "dogen.relational/test_data/tracing/model_set_td.hpp"

BOOST_AUTO_TEST_SUITE(tracing_model_set_tests)

BOOST_AUTO_TEST_CASE(identical_objects_are_equal) {
    dogen::relational::tracing::model_set_generator g;
    const auto a(g());
    const auto b(a);
...
BOOST_AUTO_TEST_CASE(moved_objects_are_equal) {
    ...
    g();
    auto a(g());
    const auto b = dogen::relational::tracing::model_set();
    const auto c(a);
    BOOST_CHECK(a != b);
    BOOST_CHECK(a == c);
...
BOOST_AUTO_TEST_CASE(assigned_objects_are_equal) {
    ...
    g();
    const auto a(g());
    dogen::relational::tracing::model_set b;
    BOOST_CHECK(a != b);
...
BOOST_AUTO_TEST_CASE(copy_constructed_objects_are_equal) {
    ...
    b = dogen::relational::tracing::model_set();
    BOOST_CHECK(a != b);
}
