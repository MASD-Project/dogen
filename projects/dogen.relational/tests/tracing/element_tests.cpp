...
#include "dogen.relational/io/tracing/element_io.hpp"
#include "dogen.relational/types/tracing/element.hpp"
#include "dogen.relational/test_data/tracing/element_td.hpp"

BOOST_AUTO_TEST_SUITE(tracing_element_tests)

BOOST_AUTO_TEST_CASE(identical_objects_are_equal) {
    dogen::relational::tracing::element_generator g;
    const auto a(g());
    const auto b(a);
...
BOOST_AUTO_TEST_CASE(moved_objects_are_equal) {
    ...
    g();
    auto a(g());
    const auto b = dogen::relational::tracing::element();
    const auto c(a);
    BOOST_CHECK(a != b);
    BOOST_CHECK(a == c);
...
BOOST_AUTO_TEST_CASE(assigned_objects_are_equal) {
    ...
    g();
    const auto a(g());
    dogen::relational::tracing::element b;
    BOOST_CHECK(a != b);
...
BOOST_AUTO_TEST_CASE(copy_constructed_objects_are_equal) {
    ...
    b = dogen::relational::tracing::element();
    BOOST_CHECK(a != b);
}
