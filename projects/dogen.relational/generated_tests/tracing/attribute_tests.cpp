...
#include "dogen.relational/io/tracing/attribute_io.hpp"
#include "dogen.relational/types/tracing/attribute.hpp"
#include "dogen.relational/test_data/tracing/attribute_td.hpp"

BOOST_AUTO_TEST_SUITE(tracing_attribute_tests)

BOOST_AUTO_TEST_CASE(identical_objects_are_equal) {
    dogen::relational::tracing::attribute_generator g;
    const auto a(g());
    const auto b(a);
...
BOOST_AUTO_TEST_CASE(moved_objects_are_equal) {
    ...
    g();
    auto a(g());
    const auto b = dogen::relational::tracing::attribute();
    const auto c(a);
    BOOST_CHECK(a != b);
    BOOST_CHECK(a == c);
...
BOOST_AUTO_TEST_CASE(assigned_objects_are_equal) {
    ...
    g();
    const auto a(g());
    dogen::relational::tracing::attribute b;
    BOOST_CHECK(a != b);
...
BOOST_AUTO_TEST_CASE(copy_constructed_objects_are_equal) {
    ...
    b = dogen::relational::tracing::attribute();
    BOOST_CHECK(a != b);
}
