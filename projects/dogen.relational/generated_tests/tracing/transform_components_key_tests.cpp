...
#include "dogen.relational/io/tracing/transform_components_key_io.hpp"
...
BOOST_AUTO_TEST_SUITE(tracing_transform_components_key_tests)

BOOST_AUTO_TEST_CASE(identical_objects_are_equal) {
    dogen::relational::tracing::transform_components_key_generator g;
    const auto a(g());
    const auto b(a);
...
BOOST_AUTO_TEST_CASE(moved_objects_are_equal) {
    ...
    g();
    auto a(g());
    const auto b = dogen::relational::tracing::transform_components_key();
    const auto c(a);
    BOOST_CHECK(a != b);
    BOOST_CHECK(a == c);
...
BOOST_AUTO_TEST_CASE(assigned_objects_are_equal) {
    ...
    g();
    const auto a(g());
    dogen::relational::tracing::transform_components_key b;
    BOOST_CHECK(a != b);
...
BOOST_AUTO_TEST_CASE(copy_constructed_objects_are_equal) {
    ...
    b = dogen::relational::tracing::transform_components_key();
    BOOST_CHECK(a != b);
}
