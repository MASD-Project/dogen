...
using factory = dogen::external::dia::test::mock_processed_object_factory;
...
bool is_root_id(const std::string id) {
    return id == dogen::external::dia::grapher::root_id();
}

class visitor : public boost::default_dfs_visitor {
public:
    visitor() : objects_(new std::list<dogen::external::dia::processed_object>()) {}
...
public:
    ...
    const std::list<dogen::external::dia::processed_object>& result() {
        return *objects_;
    }

private:
    std::shared_ptr<std::list<dogen::external::dia::processed_object>> objects_;
};

}

using dogen::utility::test::contains_checker;
using dogen::external::dia::graphing_error;
