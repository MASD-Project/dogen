...
masd::dogen::tracing::metrics*
create_masd_dogen_tracing_metrics_ptr(const unsigned int) {
    return nullptr;
}

boost::shared_ptr<masd::dogen::tracing::metrics>
create_boost_shared_ptr_masd_dogen_tracing_metrics(unsigned int position) {
    boost::shared_ptr<masd::dogen::tracing::metrics> r(
        create_masd_dogen_tracing_metrics_ptr(position));
    return r;
}

std::list<boost::shared_ptr<masd::dogen::tracing::metrics> > create_std_list_boost_shared_ptr_masd_dogen_tracing_metrics(unsigned int position) {
    std::list<boost::shared_ptr<masd::dogen::tracing::metrics> > r;
    for (unsigned int i(0); i < 4; ++i) {
        r.push_back(create_boost_shared_ptr_masd_dogen_tracing_metrics(position + i));
    }
    return r;
}
...
populate(const unsigned int position, result_type& v) {
    ...
    v.children(create_std_list_boost_shared_ptr_masd_dogen_tracing_metrics(position + 5));
}
