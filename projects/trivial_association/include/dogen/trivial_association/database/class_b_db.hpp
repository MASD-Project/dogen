 ...
 *
 */
#ifndef DOGEN_TRIVIAL_ASSOCIATION_DATABASE_CLASS_B_DB_HPP
#define DOGEN_TRIVIAL_ASSOCIATION_DATABASE_CLASS_B_DB_HPP
...
private:
    ...
    std::vector<dogen::trivial_association::class_b> load_internal(
...
public:
    std::vector<dogen::trivial_association::class_b> load(
        pqxx::connection& connection,
        const boost::optional<versioned_key> id =boost::optional<versioned_key>());

    void save(
        pqxx::connection& connection,
        std::vector<dogen::trivial_association::class_b> values);
