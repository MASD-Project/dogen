...
#include "masd.dogen.engine/io/helpers/stereotypes_conversion_result_io.hpp"
#include "masd.dogen.engine/types/helpers/stereotypes_conversion_result.hpp"
#include "masd.dogen.engine/test_data/helpers/stereotypes_conversion_result_td.hpp"

BOOST_AUTO_TEST_SUITE(helpers_stereotypes_conversion_result_tests)

BOOST_AUTO_TEST_CASE(identical_objects_are_equal) {
    masd::dogen::engine::helpers::stereotypes_conversion_result_generator g;
    const auto a(g());
    const auto b(a);
...
BOOST_AUTO_TEST_CASE(moved_objects_are_equal) {
    ...
    g();
    auto a(g());
    const auto b = masd::dogen::engine::helpers::stereotypes_conversion_result();
    const auto c(a);
    BOOST_CHECK(a != b);
    BOOST_CHECK(a == c);
...
BOOST_AUTO_TEST_CASE(assigned_objects_are_equal) {
    ...
    g();
    const auto a(g());
    masd::dogen::engine::helpers::stereotypes_conversion_result b;
    BOOST_CHECK(a != b);
...
BOOST_AUTO_TEST_CASE(copy_constructed_objects_are_equal) {
    ...
    b = masd::dogen::engine::helpers::stereotypes_conversion_result();
    BOOST_CHECK(a != b);
}
