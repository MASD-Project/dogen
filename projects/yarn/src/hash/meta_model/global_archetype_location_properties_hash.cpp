...
inline std::size_t hash_std_unordered_map_std_string_dogen_yarn_meta_model_archetype_properties(const std::unordered_map<std::string, dogen::yarn::meta_model::archetype_properties>& v) {
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i.first);
        combine(seed, i.second);
...
inline std::size_t hash_std_unordered_map_std_string_dogen_yarn_meta_model_denormalised_archetype_properties(const std::unordered_map<std::string, dogen::yarn::meta_model::denormalised_archetype_properties>& v) {
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i.first);
        combine(seed, i.second);
...
std::size_t global_archetype_location_properties_hasher::hash(const global_archetype_location_properties& v) {
    ...
    combine(seed, hash_std_unordered_map_std_string_dogen_yarn_meta_model_denormalised_archetype_properties(v.denormalised_archetype_properties()));
