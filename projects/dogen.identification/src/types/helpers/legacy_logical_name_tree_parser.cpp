...
auto lg(logger_factory(
        "identification.helpers.legacy_legacy_name_tree_parser"));
...
struct grammar : qi::grammar<Iterator> {
    boost::shared_ptr<logical_name_tree_builder> builder;
    ...
    grammar(boost::shared_ptr<logical_name_tree_builder> b,
...
legacy_logical_name_tree_parser(const entities::technical_space ts)
    : technical_space_(ts) {}

entities::logical_name_tree
legacy_logical_name_tree_parser::parse(const std::string& s) const {
    BOOST_LOG_SEV(lg, debug) << "parsing name: " << s;

    auto builder(boost::make_shared<logical_name_tree_builder>());
