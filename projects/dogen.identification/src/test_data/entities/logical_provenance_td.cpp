...
logical_provenance_generator::logical_provenance_generator() : position_(0) { }

void logical_provenance_generator::
populate(const unsigned int position, result_type& v) {
    v.codec(create_dogen_identification_entities_codec_provenance(position + 0));
    v.logical_name(create_dogen_identification_entities_logical_name(position + 1));
    v.logical_meta_name(create_dogen_identification_entities_logical_meta_name(position + 2));
}

logical_provenance_generator::result_type
logical_provenance_generator::create(const unsigned int position) {
    logical_provenance r;
    logical_provenance_generator::populate(position, r);
    return r;
}

logical_provenance_generator::result_type*
logical_provenance_generator::create_ptr(const unsigned int position) {
    logical_provenance* p = new logical_provenance();
    logical_provenance_generator::populate(position, *p);
    return p;
}

logical_provenance_generator::result_type
logical_provenance_generator::operator()() {
