...
/**
 * @brief Dia to SML is a unidirectional transformation model that transforms a
 * Dia @e diagram into an SML @e model.
 *
 * By that we mean that it is a model designed to transform a representation of
 * a user model in Dia diagram format to its counterpart in SML format, taking
 * into account any special SML meta-data which the user may have encoded into
 * the Dia diagram.Note that the Dia diagram format contains a representation
 * of UML.
 *
 * @section dia_to_sml_0 Dia diagrams as DAGs
 *
 ...
 * @li UML notes
 *
 * and so on. Each of these objects will contribute to a SML object; but note
 * that the mapping is not one-to-one.For example, in order to process an
 * SML object, one needs to have both the UML class as well as any
 ...
 * an SML transformation anyway so not much was lost (instead, one must
 * manually declare attributes in a class).
 *
 * See @ref grapher for details on how the the DAG is built.
 *
 * @section dia_to_sml_1 Intermediate processing
 *
 * To make the code easier to read and maintain, we created a numeber of
 * intermediate stages between a Dia diagram and SML:
 *
 * @li Dia object -> processed object -> profile -> SML
 *
 ...
 * build a profile. A profile allows us to quickly scan for UML and SML
 ...
 * create the corresponding SML entity, and the transformation code is
 * not obscured.
 *
 * @section dia_to_sml_2 Dia meta-data
 *
 ...
 * expexted.
 *
 * The SML stereotypes have the following behaviours:
 *
