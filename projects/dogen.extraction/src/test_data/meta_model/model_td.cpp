 ...
 */
#include <sstream>
#include "dogen.extraction/test_data/meta_model/model_td.hpp"
#include "dogen.extraction/test_data/meta_model/artefact_td.hpp"
...
create_dogen_extraction_meta_model_artefact(const unsigned int position) {
    return dogen::extraction::meta_model::artefact_generator::create(position);
}

std::list<dogen::extraction::meta_model::artefact> create_std_list_dogen_extraction_meta_model_artefact(unsigned int position) {
    std::list<dogen::extraction::meta_model::artefact> r;
    for (unsigned int i(0); i < 4; ++i) {
        r.push_back(create_dogen_extraction_meta_model_artefact(position + i));
    }
    return r;
}
...
dogen::extraction::meta_model::outputting_properties
create_dogen_extraction_meta_model_outputting_properties(const unsigned int position) {
    return dogen::extraction::meta_model::outputting_properties_generator::create(position);
}
...
populate(const unsigned int position, result_type& v) {
    v.configuration(create_boost_shared_ptr_dogen_variability_meta_model_configuration(position + 0));
    v.name(create_std_string(position + 1));
    v.technical_space(create_std_string(position + 2));
    v.artefacts(create_std_list_dogen_extraction_meta_model_artefact(position + 3));
    v.managed_directories(create_std_list_boost_filesystem_path(position + 4));
    v.outputting_properties(create_dogen_extraction_meta_model_outputting_properties(position + 5));
}
