...
using dogen::generation::formatters::boilerplate_properties;
...
std::string format(const boilerplate_properties& bp) {
    ...
    dogen::generation::formatters::indent_filter::push(fo, 4);
    fo.push(s);

    dogen::generation::formatters::boilerplate_formatter f(fo, bp);
    f.format_begin();
    f.format_end();
    return s.str();
}
