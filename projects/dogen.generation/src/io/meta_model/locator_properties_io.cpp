...
inline std::ostream& operator<<(std::ostream& s, const std::unordered_map<std::string, dogen::generation::meta_model::archetype_family_properties>& v) {
    s << "[";
    for (auto i(v.begin()); i != v.end(); ++i) {
        if (i != v.begin()) s << ", ";
        s << "[ { " << "\"__type__\": " << "\"key\"" << ", " << "\"data\": ";
...
std::ostream& operator<<(std::ostream& s, const locator_properties& v) {
    ...
    s << " { "
      << "\"__type__\": " << "\"dogen::generation::meta_model::locator_properties\"" << ", "
