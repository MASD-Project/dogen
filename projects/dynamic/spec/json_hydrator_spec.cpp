...
const std::string test_module("dynamic");
...
std::list<dogen::dynamic::field_definition>
hydrate(std::istream& s) {
    dogen::dynamic::json_hydrator h;
    return h.hydrate(s);
}
...
BOOST_AUTO_TEST_CASE(trivial_field_definition_hydrates_into_expected_collection) {
    ...
    const auto st(dogen::dynamic::scope_types::not_applicable);
    BOOST_CHECK(fd.scope() == st);

    const auto vt(dogen::dynamic::value_types::boolean);
