...
inline std::size_t hash_boost_shared_ptr_dogen_dynamic_schema_value(const boost::shared_ptr<dogen::dynamic::schema::value>& v){
    std::size_t seed(0);
    combine(seed, *v);
    return seed;
}
...
std::size_t field_definition_hasher::hash(const field_definition&v) {
    ...
    combine(seed, hash_boost_shared_ptr_dogen_dynamic_schema_value(v.default_value()));
