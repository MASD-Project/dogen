...
inline std::size_t hash_std_unordered_map_std_string_std_list_dogen_dynamic_schema_field_definition_(const std::unordered_map<std::string, std::list<dogen::dynamic::schema::field_definition> >& v){
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i.first);
        combine(seed, hash_std_list_dogen_dynamic_schema_field_definition(i.second));
    }
    return seed;
}
