...
const std::string transform_id("text.cpp.transforms.build.source_cmakelists_transform");
...
boost::filesystem::path source_cmakelists_transform::full_path(
    const formattables::locator& l, const identification::entities::logical_name& n) const {
    return l.make_full_path_for_source_cmakelists(n, archetype().meta_name().id().value());
}
