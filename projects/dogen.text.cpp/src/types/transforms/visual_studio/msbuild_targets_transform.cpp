...
const std::string transform_id("text.cpp.transforms.visual_studio.msbuild_targets_transform");
...
boost::filesystem::path msbuild_targets_transform::full_path(
    const formattables::locator& l, const logical::entities::name& n) const {
    return l.make_full_path_for_msbuild_targets(n, archetype().meta_name().id().value());
}
