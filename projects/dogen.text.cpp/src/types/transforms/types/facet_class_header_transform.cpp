...
const std::string transform_id("text.cpp.transforms.types.facet_class_header_transform");
...
boost::filesystem::path facet_class_header_transform::full_path(
    const formattables::locator& l, const logical::entities::name& n) const {
    return l.make_full_path_for_cpp_header(n, archetype().meta_name().qualified());
}

std::list<std::string> facet_class_header_transform::inclusion_dependencies(
...
void facet_class_header_transform::apply(const context& ctx, const logical::entities::element& e,
