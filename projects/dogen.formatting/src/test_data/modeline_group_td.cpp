...
std::unordered_map<std::string, dogen::formatting::modeline> create_std_unordered_map_std_string_dogen_formatting_modeline(unsigned int position) {
    std::unordered_map<std::string, dogen::formatting::modeline> r;
    for (unsigned int i(0); i < 4; ++i) {
        r.insert(std::make_pair(create_std_string(position + i), create_dogen_formatting_modeline(position + i)));
    }
    return r;
}
...
populate(const unsigned int position, result_type& v) {
    ...
    v.modelines(create_std_unordered_map_std_string_dogen_formatting_modeline(position + 1));
}
