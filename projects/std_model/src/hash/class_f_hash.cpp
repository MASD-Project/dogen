...
inline std::size_t hash_std_unordered_map_dogen_std_model_class_a_dogen_std_model_pkg1_class_c(const std::unordered_map<dogen::std_model::class_a, dogen::std_model::pkg1::class_c>& v){
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i.first);
        combine(seed, i.second);
