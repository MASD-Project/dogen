...
std::unordered_map<std::string, dogen::annotations::profile> create_std_unordered_map_std_string_dogen_annotations_profile(unsigned int position) {
    std::unordered_map<std::string, dogen::annotations::profile> r;
    for (unsigned int i(0); i < 4; ++i) {
        r.insert(std::make_pair(create_std_string(position + i), create_dogen_annotations_profile(position + i)));
    }
    return r;
}
