...
inline std::size_t hash_std_vector_masd_dogen_coding_meta_model_enumerator(const std::vector<masd::dogen::coding::meta_model::enumerator>& v) {
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i);
    }
...
std::size_t enumeration_hasher::hash(const enumeration& v) {
    ...
    combine(seed, hash_std_vector_masd_dogen_coding_meta_model_enumerator(v.enumerators()));
