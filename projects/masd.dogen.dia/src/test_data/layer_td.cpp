...
create_masd_dogen_dia_object(const unsigned int position) {
    return masd::dogen::dia::object_generator::create(position);
}

std::vector<masd::dogen::dia::object> create_std_vector_masd_dogen_dia_object(unsigned int position) {
    std::vector<masd::dogen::dia::object> r;
    for (unsigned int i(0); i < 4; ++i) {
        r.push_back(create_masd_dogen_dia_object(position + i));
    }
    return r;
}
...
populate(const unsigned int position, result_type& v) {
    ...
    v.objects(create_std_vector_masd_dogen_dia_object(position + 3));
}
