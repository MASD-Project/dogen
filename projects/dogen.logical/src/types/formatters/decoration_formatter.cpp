...
#include "dogen.logical/types/formatters/modeline_formatter.hpp"
...
auto lg(logger_factory("logical.decoration_formatter"));
...
add_modeline(std::list<std::string>& content,
    const boost::shared_ptr<entities::decoration::modeline> ml
    ) const {
    if (!ml) {
...
add_marker(std::list<std::string>& content,
    ...
    entities::decoration::generation_marker> gm) const {
    if (!gm) {
...
void decoration_formatter::format_preamble(
    ...
    const utility::formatters::comment_style& multi_line_cs,
    ...
    const auto top(entities::decoration::modeline_location::top);
...
format_postamble(std::ostream& s, const utility::formatters::comment_style& cs,
    const boost::shared_ptr<entities::decoration::modeline> ml) const {
