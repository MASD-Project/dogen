...
auto lg(logger_factory("logical.helpers.resolver"));
...
void resolver::resolve_name_tree(const entities::model& m,
    ...
    identification::helpers::qualified_representations_builder b;
    b.add(n);
    ...
    for (auto& pair : m.structural_elements().enumerations()) {
        ...
        if (i == idx.enumeration_underliers().end()) {
            ...
            BOOST_THROW_EXCEPTION(
                resolution_error(invalid_underlying_type + ue_id.value()));
        }
...
void resolver::resolve_archetypes(const indices& idx, entities::model& m) {
    for (auto& pair : m.physical_elements().archetypes()) {
        ...
        arch.text_templating().wale_template(resolved);
    }
}
