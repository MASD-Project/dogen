...
inline std::ostream& operator<<(std::ostream& s, const std::unordered_map<dogen::logical::entities::orm::database_system, std::string>& v) {
    s << "[";
    for (auto i(v.begin()); i != v.end(); ++i) {
        if (i != v.begin()) s << ", ";
        s << "[ { " << "\"__type__\": " << "\"key\"" << ", " << "\"data\": ";
...
std::ostream& operator<<(std::ostream& s, const primitive_properties& v) {
    ...
    s << " { "
      << "\"__type__\": " << "\"dogen::logical::entities::orm::primitive_properties\"" << ", "
