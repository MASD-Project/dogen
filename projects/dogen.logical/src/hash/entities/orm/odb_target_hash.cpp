...
std::size_t odb_target_hasher::hash(const odb_target& v) {
    std::size_t seed(0);

    combine(seed, v.name());
    combine(seed, v.comment());
    combine(seed, v.output_directory());
    combine(seed, v.types_file());
    combine(seed, hash_std_list_std_pair_std_string_std_string(v.move_parameters()));
    combine(seed, v.object_odb_options());
