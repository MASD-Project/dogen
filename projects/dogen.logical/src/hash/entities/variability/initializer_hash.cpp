...
std::size_t initializer_hasher::hash(const initializer& v) {
    std::size_t seed(0);

    combine(seed, dynamic_cast<const dogen::logical::entities::element&>(v));

    combine(seed, hash_std_list_dogen_identification_entities_logical_name(v.feature_template_bundles()));
    combine(seed, hash_std_list_dogen_identification_entities_logical_name(v.feature_bundles()));
