 ...
 *
 */
#ifndef DOGEN_LOGICAL_TYPES_HELPERS_NEW_NAME_TREE_PARSER_HPP
#define DOGEN_LOGICAL_TYPES_HELPERS_NEW_NAME_TREE_PARSER_HPP
...
struct name_tree_listener
{
    ...
    */
    ...
    name_tree_listener(const std::unordered_set<std::string>& top_level_modules,
        const dogen::logical::meta_model::location& model_location)
...
struct custom_type_grammar : qi::grammar<Iterator, Skipper>
{
    ...
    std::string scope_operator_for_technical_space(
        const dogen::logical::meta_model::technical_space ts) {
        switch (ts) {
        case dogen::logical::meta_model::technical_space::csharp:
            return ".";
        case dogen::logical::meta_model::technical_space::cpp:
            return "::";
        default: {
            const auto s(boost::lexical_cast<std::string>(ts));
            BOOST_LOG_SEV(lg, error) << unsupported_technical_space << s;
            BOOST_THROW_EXCEPTION(
                dogen::logical::helpers::parsing_error(
