...
inline std::size_t hash_std_vector_dogen_tack_enumerator(const std::vector<dogen::tack::enumerator>& v) {
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i);
    }
...
std::size_t enumeration_hasher::hash(const enumeration& v) {
    ...
    combine(seed, hash_std_vector_dogen_tack_enumerator(v.enumerators()));
