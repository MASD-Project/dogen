...
#include "dogen/test_models/database/types/all.hpp"
#include "dogen/test_models/database/io/all_io.hpp"
#include "dogen/test_models/database/test_data/all_td.hpp"
...
const std::string test_suite("database_tests");
...
BOOST_AUTO_TEST_CASE(inserting_no_keys_instances_results_in_expected_rows_in_table) {
    ...
    const auto del_rows(delete_rows<dogen::test_models::database::no_keys>(*db));
    BOOST_LOG_SEV(lg, debug) << "Deleted existing rows. Total: " << del_rows;

    const unsigned int how_many(5);
    const auto v(generate<dogen::test_models::database::no_keys_generator>(how_many));
    persist(*db, v);
    BOOST_LOG_SEV(lg, debug) << "Persisted: " << v;

    {
        transaction t(db->begin());
        typedef odb::result<dogen::test_models::database::no_keys> result;

        result r(db->query<dogen::test_models::database::no_keys>());
...
BOOST_AUTO_TEST_CASE(inserting_no_keys_2_instances_results_in_expected_rows_in_table) {
    ...
    std::unique_ptr<odb::database> db(create_db());
    const auto del_rows(delete_rows<dogen::test_models::database::no_keys_2>(*db));
    BOOST_LOG_SEV(lg, debug) << "Deleted existing rows. Total: " << del_rows;

    const unsigned int how_many(5);
    const auto v(generate<dogen::test_models::database::no_keys_2_generator>(how_many));
    persist(*db, v);
    BOOST_LOG_SEV(lg, debug) << "Persisted: " << v;

    {
        transaction t(db->begin());
        typedef odb::result<dogen::test_models::database::no_keys_2> result;

        result r(db->query<dogen::test_models::database::no_keys_2>());
        for (auto i(r.begin ()); i != r.end (); ++i) {
            BOOST_LOG_SEV(lg, debug) << "Actual: " << *i;
            bool found(false);
            for (const auto e : v) {
...
BOOST_AUTO_TEST_CASE(inserting_primary_key_objects_results_in_unique_rows) {
    ...
    std::unique_ptr<odb::database> db(create_db());
    const auto del_rows(delete_rows<dogen::test_models::database::primary_key>(*db));
    BOOST_LOG_SEV(lg, debug) << "Deleted existing rows. Total: " << del_rows;

    const unsigned int how_many(5);
    const auto v(generate<dogen::test_models::database::primary_key_generator>(how_many));
    persist(*db, v);
    BOOST_LOG_SEV(lg, debug) << "Persisted: " << v;

    {
        ...
        const auto a(db->load<dogen::test_models::database::primary_key>(e.prop_0()));
...

    {
        transaction t(db->begin());
        auto e(v.back());
        auto a(db->load<dogen::test_models::database::primary_key>(e.prop_0()));
...
BOOST_AUTO_TEST_CASE(inserting_foreign_key_objects_requires_other_side_to_exist) {
    ...
    std::unique_ptr<odb::database> db(create_db());
    auto del_rows(delete_rows<dogen::test_models::database::foreign_key>(*db));
    BOOST_LOG_SEV(lg, debug) << "Deleted rows in foreign_key. Total: "
                             << del_rows;

    del_rows = delete_rows<dogen::test_models::database::primary_key_2>(*db);
    BOOST_LOG_SEV(lg, debug) << "Deleted rows in primary_key_2. Total: "
                             << del_rows;

    const unsigned int how_many(5);
    const auto v(generate<dogen::test_models::database::primary_key_2_generator>(how_many));
    persist(*db, v);
    BOOST_LOG_SEV(lg, debug) << "Persisted: " << v;

    {
        odb::transaction t(db->begin());
        dogen::test_models::database::foreign_key fk;
        fk.prop_0(15);
        boost::shared_ptr<dogen::test_models::database::primary_key_2>
            sp(new dogen::test_models::database::primary_key_2(v.back()));
        fk.prop_1(sp);
        db->persist(fk);
    ...
    {
        transaction t(db->begin());
        typedef odb::result<dogen::test_models::database::foreign_key> result;

        result r(db->query<dogen::test_models::database::foreign_key>());
        for (auto i(r.begin ()); i != r.end (); ++i) {
            BOOST_LOG_SEV(lg, debug) << "Actual: " << *i;
            bool found(false);
            if (v.back() == *(i->prop_1())) {
