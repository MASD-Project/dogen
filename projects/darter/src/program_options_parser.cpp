...
options::darting_options program_options_parser::
...
boost::optional<options::darting_options> program_options_parser::parse() {
    auto optional_vm(variables_map_factory());

    if (!optional_vm)
        return boost::optional<options::darting_options>();

    const boost::program_options::variables_map vm(*optional_vm);
    const auto r(transform_options(vm));
    return boost::optional<options::darting_options>(r);
}
