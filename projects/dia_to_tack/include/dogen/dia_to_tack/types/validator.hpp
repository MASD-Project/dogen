 ...
 *
 */
#ifndef DOGEN_DIA_TO_TACK_TYPES_VALIDATOR_HPP
#define DOGEN_DIA_TO_TACK_TYPES_VALIDATOR_HPP
 ...
 *
 * @li have zero or one stereotype with Tack type information, such as
 * enumeration, value object, etc. The Tack type must be compatible
 * with the UML type.
 *
 ...
 * than making sure that they belong to Tack types which support them.
 */
class validator {
public:
    validator() = default;
...
private:
    /**
     * @brief Returns true if the Tack flags with type information
...
private:
    /**
     * @brief Returns the number of flags that are options on Tack
     * objects.
     *
     * By @e object we mean any descendent of @ref object.
     */
    unsigned int count_tack_object_flags(const profile& p) const;

    /**
     * @brief Returns the number of flags that convey type information
     * on Tack modeling elements.
     */
    unsigned int count_tack_types(const profile& p) const;
...
private:
    /**
     * @brief Checks all flags that are related to Tack.
     */
    void validate_tack(const profile& p) const;
