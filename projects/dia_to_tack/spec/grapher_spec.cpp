...
#include "dogen/dia_to_tack/io/processed_object_io.hpp"
...
bool is_root_id(const std::string id) {
    return id == dogen::dia_to_tack::grapher::root_id();
}

class visitor : public boost::default_dfs_visitor {
public:
    explicit visitor(std::list<dogen::dia_to_tack::processed_object>& o)
        : objects_(o) {}
...
private:
    std::list<dogen::dia_to_tack::processed_object>& objects_;
};

}

using dogen::utility::test::contains_checker;
using dogen::dia_to_tack::graphing_error;
...
BOOST_AUTO_TEST_CASE(not_adding_objects_to_graph_produces_only_root_object) {
    ...
    std::list<dogen::dia_to_tack::processed_object> o;
    visitor v(o);

    dogen::dia_to_tack::grapher g;
    g.generate();
...
BOOST_AUTO_TEST_CASE(adding_object_after_graph_has_been_generated_throws) {
    ...
    dogen::dia_to_tack::grapher g1;
    g1.generate();
    const auto o(mock_processed_object_factory::make_class(0));
    contains_checker<graphing_error> c(graph_generated);
    BOOST_CHECK_EXCEPTION(g1.add(o), graphing_error, c);

    dogen::dia_to_tack::grapher g2;
    g2.add(o);
    g2.generate();
