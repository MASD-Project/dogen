...
#include "dogen/dia_to_tack/types/transformer.hpp"
#include "dogen/dia_to_tack/types/profiler.hpp"
#include "dogen/dia_to_tack/types/transformation_error.hpp"
#include "dogen/dia_to_tack/io/context_io.hpp"
#include "dogen/dia_to_tack/types/processed_object.hpp"
#include "dogen/dia_to_tack/test/mock_processed_object_factory.hpp"
#include "dogen/dynamic/test/mock_repository_factory.hpp"
#include "dogen/dynamic/test/mock_workflow_factory.hpp"
#include "dogen/utility/test/exception_checkers.hpp"

using namespace dogen::dia_to_tack;
using dogen::utility::test::asserter;
using dogen::dia_to_tack::test::mock_processed_object_factory;
...
dogen::dia_to_tack::profile
mock_profile(const dogen::dia_to_tack::processed_object& o) {
...
dogen::dia_to_tack::context mock_context(const std::string& model_name) {
    dogen::dia_to_tack::context r;
...
dogen::dia_to_tack::context mock_context() {
...
void transform(dogen::dia_to_tack::context& c,
    std::initializer_list<dogen::dia_to_tack::processed_object> lpo) {
    ...
    dogen::dia_to_tack::transformer t(w, c);
...
void transform(dogen::dia_to_tack::context& c,
    const dogen::dia_to_tack::processed_object& po,
    const dogen::dia_to_tack::profile& p) {
...
BOOST_AUTO_TEST_CASE(uml_class_with_exception_stereotype_transforms_into_expected_value_object) {
    ...
    BOOST_CHECK(e.object_type() == dogen::tack::object_types::exception);
    BOOST_CHECK(e.name().model_name() == model_name);
    BOOST_CHECK(is_type_zero(e.name()));
    BOOST_CHECK(!e.documentation().empty());
...
BOOST_AUTO_TEST_CASE(uml_class_with_service_stereotype_transforms_into_expected_service) {
    ...
    const auto& o(c.model().objects().begin()->second);
    BOOST_CHECK(o.object_type() ==
        dogen::tack::object_types::user_defined_service);
    BOOST_CHECK(o.name().model_name() == model_name);
    BOOST_CHECK(is_type_zero(o.name()));
    BOOST_CHECK(!o.documentation().empty());
...
BOOST_AUTO_TEST_CASE(uml_class_with_exception_stereotype_in_package_transforms_into_expected_exception) {
    ...
    BOOST_REQUIRE(c.model().objects().size() == 1);
    const auto& o(c.model().objects().begin()->second);
    BOOST_CHECK(o.object_type() == dogen::tack::object_types::exception);
    BOOST_CHECK(o.name().model_name() == model_name);
    BOOST_CHECK(!o.name().simple_name().empty());
    BOOST_CHECK(module_member == o.name().simple_name());
    BOOST_REQUIRE(o.name().module_path().size() == 1);
...
BOOST_AUTO_TEST_CASE(uml_class_in_two_packages_transforms_into_expected_object) {
    ...
    std::list<dogen::tack::module> module_list;
    for (const auto& pair : c.model().modules()) {
        const auto& m(pair.second);
        if (m.name().simple_name() != model_name)
