 ...
 *
 */
#include "dogen/dia_to_tack/hash/object_types_hash.hpp"
#include "dogen/dia_to_tack/hash/processed_object_hash.hpp"
#include "dogen/dia_to_tack/hash/processed_comment_hash.hpp"
#include "dogen/dia_to_tack/hash/processed_property_hash.hpp"
...
inline std::size_t hash_std_list_dogen_dia_to_tack_processed_property(const std::list<dogen::dia_to_tack::processed_property>& v) {
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i);
    }
...
std::size_t processed_object_hasher::hash(const processed_object& v) {
    ...
    combine(seed, hash_std_list_dogen_dia_to_tack_processed_property(v.properties()));
