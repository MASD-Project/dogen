...
static logger lg(logger_factory("dia_to_tack.validator"));
...
const std::string too_many_tack_types("Can only have one TACK type set.");
...
unsigned int validator::count_tack_object_flags(const profile& p) const {
    unsigned int r(0);
...
unsigned int validator::count_tack_types(const profile& p) const {
    unsigned int r(0);
...
void validator::validate_tack(const profile& p) const {
    const auto types(count_tack_types(p));
    const auto object_flags(count_tack_object_flags(p));
    const bool has_tack_flags(
    ...
    /*
     * only UML classes are allowed to have TACK flags.
     */
    if (!p.is_uml_class()) {
    ...
    /*
     *  we can have at most one TACK type. Zero is fine as
     * someone above us will provide defaulting.
     */
    if (types > 1) {
        BOOST_LOG_SEV(lg, error) << too_many_tack_types;
        BOOST_THROW_EXCEPTION(validation_error(too_many_tack_types));
    }

    if (is_object(p))
        return; // nothing else to validate for TACK objects.

    /*
     * non TACK objects are not allowed to have object flags.
     */
    if (object_flags > 0) {
    ...
    /*
     *  non TACK objects are not allowed to have concepts.
     */
    if (!p.unknown_stereotypes().empty()) {
        BOOST_LOG_SEV(lg, error) << concepts_require_tack_object << ": "
                                 << p.unknown_stereotypes();
        BOOST_THROW_EXCEPTION(validation_error(concepts_require_tack_object));
    }
}
