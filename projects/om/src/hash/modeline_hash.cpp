...
inline std::size_t hash_std_list_dogen_om_modeline_field(const std::list<dogen::om::modeline_field>& v){
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i);
    }
...
std::size_t modeline_hasher::hash(const modeline&v) {
    ...
    combine(seed, hash_std_list_dogen_om_modeline_field(v.fields()));
