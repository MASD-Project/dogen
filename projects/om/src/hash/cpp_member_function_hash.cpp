...
std::size_t cpp_member_function_hasher::hash(const cpp_member_function&v) {
    std::size_t seed(0);

    combine(seed, dynamic_cast<const dogen::om::cpp_abstract_function&>(v));

    combine(seed, v.is_virtual());
    combine(seed, v.is_explicit());
    combine(seed, v.is_pure());
    combine(seed, v.is_override());
    combine(seed, v.is_final());
    combine(seed, v.is_default());
    combine(seed, v.is_static());
    combine(seed, v.type());
