 ...
 *
 */
#ifndef DOGEN_FORMATTERS_TYPES_FILE_ANNOTATIONS_HPP
#define DOGEN_FORMATTERS_TYPES_FILE_ANNOTATIONS_HPP
...
#include "dogen/formatters/serialization/file_annotations_fwd_ser.hpp"

namespace dogen {
namespace formatters {

class file_annotations final {
public:
    file_annotations() = default;
    file_annotations(const file_annotations&) = default;
    ~file_annotations() = default;

public:
    file_annotations(file_annotations&& rhs);
...
private:
    ...
    friend void boost::serialization::save(Archive& ar, const dogen::formatters::file_annotations& v, unsigned int version);

    template<typename Archive>
    friend void boost::serialization::load(Archive& ar, dogen::formatters::file_annotations& v, unsigned int version);
...
public:
    bool operator==(const file_annotations& rhs) const;
    bool operator!=(const file_annotations& rhs) const {
        return !this->operator==(rhs);
    }

public:
    void swap(file_annotations& other) noexcept;
    file_annotations& operator=(file_annotations other);
...
template<>
inline void swap(
    dogen::formatters::file_annotations& lhs,
    dogen::formatters::file_annotations& rhs) {
    lhs.swap(rhs);
}
