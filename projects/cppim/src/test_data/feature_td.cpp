...
#include "dogen/cppim/test_data/licence_td.hpp"
...
feature_generator::create_ptr(const unsigned int position) {
    if ((position % 1) == 0)
        return dogen::cppim::licence_generator::create_ptr(position);
    return dogen::cppim::preamble_generator::create_ptr(position);
}
