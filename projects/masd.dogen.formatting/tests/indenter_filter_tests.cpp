...
BOOST_AUTO_TEST_CASE(positive_indenter_scope_correctly_increases_and_decreases_indentation_level) {
    ...
    s.str(empty);
    {
        dogen::formatting::positive_indenter_scope pis(fo);
        fo << input << std::endl;
        fo.flush();
        BOOST_CHECK(asserter::assert_equals_marker(expected(1, 4), s.str()));
        s.str(empty);
...
BOOST_AUTO_TEST_CASE(negative_indenter_scope_correctly_decreases_and_increases_indentation_level) {
    ...
    {
        dogen::formatting::negative_indenter_scope nis(fo);
        fo << input << std::endl;
        fo.flush();
        BOOST_CHECK(asserter::assert_equals_marker(expected(1, 4), s.str()));
        s.str(empty);
...
BOOST_AUTO_TEST_CASE(single_trailing_blank_line_is_suppressed_when_managing_blank_lines) {
    ...
    fo << input << std::endl;
    fo << dogen::formatting::manage_blank_lines << std::endl;
...
BOOST_AUTO_TEST_CASE(multiple_trailing_blank_lines_are_suppressed_when_managing_blank_lines) {
    ...
    fo << input << std::endl;
    fo << dogen::formatting::manage_blank_lines << std::endl << std::endl;
    BOOST_CHECK(asserter::assert_equals_marker(without_blank_lines, s.str()));
    s.str(empty);
}
