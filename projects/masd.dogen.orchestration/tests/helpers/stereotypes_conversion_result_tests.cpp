...
BOOST_AUTO_TEST_CASE(moved_objects_are_equal) {
    ...
    g();
    auto a(g());
    const auto b = masd::dogen::orchestration::helpers::stereotypes_conversion_result();
    const auto c(a);
    BOOST_CHECK(a != b);
    BOOST_CHECK(a == c);
...
BOOST_AUTO_TEST_CASE(assigned_objects_are_equal) {
    ...
    g();
    const auto a(g());
    masd::dogen::orchestration::helpers::stereotypes_conversion_result b;
    BOOST_CHECK(a != b);
...
BOOST_AUTO_TEST_CASE(copy_constructed_objects_are_equal) {
    ...
    b = masd::dogen::orchestration::helpers::stereotypes_conversion_result();
    BOOST_CHECK(a != b);
}
