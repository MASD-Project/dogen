 ...
 *
 */
#ifndef DOGEN_SML_TEST_DATA_EXCEPTION_SEQUENCE_HPP
#define DOGEN_SML_TEST_DATA_EXCEPTION_SEQUENCE_HPP
...
public:
    result_type next_term(const unsigned int position) {
        dogen::sml::exception result;
        dogen::sml::test_data::qualified_name_sequence sequence;

        result.generate(true);
        if (position == 1) {
            result.name(sequence());
            result.documentation("exception docs 1");
        } else if (position == 2) {
            sequence();
            result.name(sequence());
            result.documentation("exception docs 2");
        } else {
            sequence();
            sequence();
            result.name(sequence());
            result.documentation("exception docs 3");
        }
        return result;
    }
...
typedef utility::test_data::sequence<
    detail::exception_generator> exception_sequence;
