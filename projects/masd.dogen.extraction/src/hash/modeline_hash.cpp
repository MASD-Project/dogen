...
inline std::size_t hash_std_list_masd_dogen_extraction_modeline_field(const std::list<masd::dogen::extraction::modeline_field>& v) {
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i);
    }
...
std::size_t modeline_hasher::hash(const modeline& v) {
    ...
    combine(seed, hash_std_list_masd_dogen_extraction_modeline_field(v.fields()));
