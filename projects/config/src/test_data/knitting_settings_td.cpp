...
#include "dogen/config/test_data/input_settings_td.hpp"
#include "dogen/config/test_data/knitting_settings_td.hpp"
...
dogen::config::input_settings
create_dogen_config_input_settings(const unsigned int position) {
    return dogen::config::input_settings_generator::create(position);
}
...
knitting_settings_generator::knitting_settings_generator() : position_(0) { }

void knitting_settings_generator::
populate(const unsigned int position, result_type& v) {
    v.verbose(create_bool(position + 0));
    v.input(create_dogen_config_input_settings(position + 1));
    ...
    v.cpp(create_dogen_config_cpp_settings(position + 4));
}

knitting_settings_generator::result_type
knitting_settings_generator::create(const unsigned int position) {
    knitting_settings r;
    knitting_settings_generator::populate(position, r);
    return r;
}
knitting_settings_generator::result_type*
knitting_settings_generator::create_ptr(const unsigned int position) {
    knitting_settings* p = new knitting_settings();
    knitting_settings_generator::populate(position, *p);
    return p;
}

knitting_settings_generator::result_type
knitting_settings_generator::operator()() {
