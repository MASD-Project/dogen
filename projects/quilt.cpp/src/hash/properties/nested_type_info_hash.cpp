...
inline std::size_t hash_std_list_dogen_quilt_cpp_properties_nested_type_info(const std::list<dogen::quilt::cpp::properties::nested_type_info>& v) {
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i);
    }
...
std::size_t nested_type_info_hasher::hash(const nested_type_info& v) {
    ...
    combine(seed, hash_std_list_dogen_quilt_cpp_properties_nested_type_info(v.children()));
