...
inline std::size_t hash_std_unordered_map_dogen_yarn_name_std_unordered_map_std_string_std_string_(const std::unordered_map<dogen::yarn::name, std::unordered_map<std::string, std::string> >& v) {
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i.first);
