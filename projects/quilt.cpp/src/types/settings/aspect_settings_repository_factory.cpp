...
static logger lg(logger_factory(
        "quilt.cpp.settings.aspect_settings_repository_factory"));
...
/**
 * @brief Generates the aspect_settings.
 */
class generator final : public yarn::element_visitor {
public:
    explicit generator(const aspect_settings_factory& f)
        : factory_(f) {}

private:
    void insert(const yarn::name& n, const settings::aspect_settings& as) {
...
private:
    const aspect_settings_factory& factory_;
    aspect_settings_repository result_;
};

}

aspect_settings_repository aspect_settings_repository_factory::
make(const dynamic::repository& rp, const yarn::model& m) const {
    BOOST_LOG_SEV(lg, debug) << "Creating aspect settings repository.";
