...
a.stream() << "    \"${include_dir}/" << odb_folder << "/*_" << c.odb_postfix() << "." << c.header_file_extension() << "\")" << std::endl;
a.stream() << std::endl;
...
a.stream() << "    COMMAND mv ${CMAKE_CURRENT_SOURCE_DIR}/include/" << product_name << "/" << model_name << "/" << odb_folder << "/*." << c.implementation_file_extension() << std::endl;
...
a.stream() << "            -I ${ODB_INCLUDE_DIRS} -I ${CMAKE_CURRENT_SOURCE_DIR}/include" << std::endl;
a.stream() << "            ${include_dir}/types/${type_name}." << c.header_file_extension() << ")" << std::endl;
a.stream() << std::endl;
