...
inline std::ostream& operator<<(std::ostream& s, const std::unordered_map<dogen::yarn::name, std::unordered_map<std::string, std::list<std::string> > >& v) {
    s << "[";
    for (auto i(v.begin()); i != v.end(); ++i) {
        if (i != v.begin()) s << ", ";
        s << "[ { " << "\"__type__\": " << "\"key\"" << ", " << "\"data\": ";
...
std::ostream& operator<<(std::ostream& s, const inclusion_dependencies_repository& v) {
    s << " { "
      << "\"__type__\": " << "\"dogen::quilt::cpp::formattables::inclusion_dependencies_repository\"" << ", "
      << "\"by_name\": " << v.by_name()
      << " }";
    return(s);
}
