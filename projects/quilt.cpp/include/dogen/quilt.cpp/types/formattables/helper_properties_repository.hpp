...
/**
 * @brief Maps a qualified name to the helper properties associated with it.
 *
 * At present we only need to know about helper family so we do not bother
 * modeling helper properties as a class.
 */
...
public:
    explicit helper_properties_repository(const std::unordered_map<std::string, std::string>& helper_properties_by_name);
