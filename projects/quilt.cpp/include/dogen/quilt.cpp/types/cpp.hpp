 ...
 *
 */
#ifndef DOGEN_QUILT_CPP_TYPES_CPP_HPP
#define DOGEN_QUILT_CPP_TYPES_CPP_HPP
 ...
 * as an yarn augmented with C++-specific information. Some of that information
 * comes from bringing yarn meta-data into the type system; the remaining is
 * directly derived by the information available in yarn.
 *
 ...
 *
 * We also tried to use yarn directly to format C++ code, augmented with
 * high-level meta-data. Since the C++ model shares so many commonalities with
 * yarn it only seeemed logical to remove it from the picture altogether and
 * add whatever was missing directly to yarn. To avoid making yarn language
 ...
 *
 * The final aproach considered was to make CPP types inherit from yarn. This
