 ...
 *
 */
#ifndef DOGEN_CPP_TYPES_CLASS_SETTINGS_HPP
#define DOGEN_CPP_TYPES_CLASS_SETTINGS_HPP
...
#include <algorithm>
#include "dogen/cpp/serialization/class_settings_fwd_ser.hpp"
 ...
 */
class class_settings final {
public:
    class_settings(const class_settings&) = default;
    class_settings(class_settings&&) = default;
    ~class_settings() = default;
...
private:
    ...
    friend void boost::serialization::save(Archive& ar, const class_settings& v, unsigned int version);

    template<typename Archive>
    friend void boost::serialization::load(Archive& ar, class_settings& v, unsigned int version);
...
public:
    bool operator==(const class_settings& rhs) const;
    bool operator!=(const class_settings& rhs) const {
        return !this->operator==(rhs);
    }

public:
    void swap(class_settings& other) noexcept;
    class_settings& operator=(class_settings other);
...
template<>
inline void swap(
    dogen::cpp::class_settings& lhs,
    dogen::cpp::class_settings& rhs) {
    lhs.swap(rhs);
}
