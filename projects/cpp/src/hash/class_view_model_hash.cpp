...
#include "dogen/cpp/hash/parent_view_model_hash.hpp"
#include "dogen/cpp/hash/property_view_model_hash.hpp"
...
inline std::size_t hash_std_list_dogen_cpp_property_view_model(const std::list<dogen::cpp::property_view_model>& v){
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i);
    }
...
inline std::size_t hash_std_list_dogen_cpp_parent_view_model(const std::list<dogen::cpp::parent_view_model>& v){
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i);
    }
...
std::size_t class_view_model_hasher::hash(const class_view_model&v) {
    ...
    combine(seed, v.name());
    combine(seed, hash_std_list_dogen_cpp_property_view_model(v.properties()));
    combine(seed, hash_std_list_dogen_cpp_property_view_model(v.all_properties()));
    ...
    combine(seed, hash_std_list_dogen_cpp_parent_view_model(v.parents()));
