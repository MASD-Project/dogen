 ...
 *
 */
#include "dogen/cpp/hash/nested_type_view_model_hash.hpp"
...
inline std::size_t hash_std_list_dogen_cpp_nested_type_view_model(const std::list<dogen::cpp::nested_type_view_model>& v){
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i);
    }
...
std::size_t nested_type_view_model_hasher::hash(const nested_type_view_model&v) {
    std::size_t seed(0);
    ...
    combine(seed, hash_std_list_dogen_cpp_nested_type_view_model(v.children()));
