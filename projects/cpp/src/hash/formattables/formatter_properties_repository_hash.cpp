...
inline std::size_t hash_std_unordered_map_std_string_std_unordered_map_std_string_dogen_cpp_formattables_formatter_properties_(const std::unordered_map<std::string, std::unordered_map<std::string, dogen::cpp::formattables::formatter_properties> >& v) {
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i.first);
...
std::size_t formatter_properties_repository_hasher::hash(const formatter_properties_repository& v) {
    ...
    combine(seed, hash_std_unordered_map_std_string_std_unordered_map_std_string_dogen_cpp_formattables_formatter_properties_(v.formatter_properties_by_name()));
    return seed;
}
