...
inline std::size_t hash_std_unordered_map_dogen_tack_name_std_unordered_map_std_string_std_list_std_string__(const std::unordered_map<dogen::tack::name, std::unordered_map<std::string, std::list<std::string> > >& v) {
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i.first);
...
std::size_t inclusion_dependencies_repository_hasher::hash(const inclusion_dependencies_repository& v) {
    ...
    combine(seed, hash_std_unordered_map_dogen_tack_name_std_unordered_map_std_string_std_list_std_string__(v.inclusion_dependencies_by_name()));
    return seed;
}
