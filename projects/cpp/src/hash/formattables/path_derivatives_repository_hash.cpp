...
inline std::size_t hash_std_unordered_map_dogen_tack_name_std_unordered_map_std_string_dogen_cpp_formattables_path_derivatives_(const std::unordered_map<dogen::tack::name, std::unordered_map<std::string, dogen::cpp::formattables::path_derivatives> >& v) {
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i.first);
...
std::size_t path_derivatives_repository_hasher::hash(const path_derivatives_repository& v) {
    ...
    combine(seed, hash_std_unordered_map_dogen_tack_name_std_unordered_map_std_string_dogen_cpp_formattables_path_derivatives_(v.path_derivatives_by_name()));
    return seed;
}
