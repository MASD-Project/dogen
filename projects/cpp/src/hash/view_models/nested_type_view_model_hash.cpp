 ...
 *
 */
#include "dogen/cpp/hash/view_models/nested_type_view_model_hash.hpp"
...
inline std::size_t hash_std_list_dogen_cpp_view_models_nested_type_view_model(const std::list<dogen::cpp::view_models::nested_type_view_model>& v){
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, i);
    }
