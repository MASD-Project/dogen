...
std::unordered_map<dogen::tack::name, std::unordered_map<std::string, bool> > create_std_unordered_map_dogen_tack_name_std_unordered_map_std_string_bool_(unsigned int position) {
    std::unordered_map<dogen::tack::name, std::unordered_map<std::string, bool> > r;
    for (unsigned int i(0); i < 4; ++i) {
        r.insert(std::make_pair(create_dogen_tack_name(position + i), create_std_unordered_map_std_string_bool(position + i)));
    }
    return r;
}
...
populate(const unsigned int position, result_type& v) {
    v.enablement_by_name(create_std_unordered_map_dogen_tack_name_std_unordered_map_std_string_bool_(position + 0));
}
