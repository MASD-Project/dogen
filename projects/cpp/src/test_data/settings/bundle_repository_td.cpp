...
std::unordered_map<std::string, dogen::cpp::settings::bundle> create_std_unordered_map_std_string_dogen_cpp_settings_bundle(unsigned int position) {
    std::unordered_map<std::string, dogen::cpp::settings::bundle> r;
    for (unsigned int i(0); i < 4; ++i) {
        r.insert(std::make_pair(create_std_string(position + i), create_dogen_cpp_settings_bundle(position + i)));
    }
    return r;
}
...
populate(const unsigned int position, result_type& v) {
    v.bundles_by_name(create_std_unordered_map_std_string_dogen_cpp_settings_bundle(position + 0));
}
