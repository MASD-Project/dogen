...
void sml_dfs_visitor::transform_nested_qualified_name(
    ...
    if (qn.meta_type() == meta_types::pod) {
        ...
        if (i == state_->pods_.end()) {
            using dogen::cpp::view_models::transformation_error;
    ...
    using dogen::cpp::view_models::nested_type_view_model;
...
create_file(config::cpp_facet_types ft, file_types flt, cpp_aspect_types at,
...
std::vector<file_view_model> sml_to_cpp_view_model::transform_pods() {
    ...
    using config::cpp_facet_types;
    auto lambda([&](cpp_facet_types ft, file_types flt, cpp_aspect_types at,
            const sml::pod& p) {
