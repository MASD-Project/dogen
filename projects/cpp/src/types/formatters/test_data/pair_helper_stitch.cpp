...
fa.stream() << "    combine(seed, test_data_" << first.complete_identifiable_name() << "(v.first));" << std::endl;

    if (!fa.requires_hashing_helper_method(second))
fa.stream() << "    combine(seed, v.second);" << std::endl;
    else
fa.stream() << "    combine(seed, test_data_" << second.complete_identifiable_name() << "(v.second));" << std::endl;
