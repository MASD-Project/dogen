...
fa.stream() << "    " << t.complete_identifiable_name() << "_visitor() : test_data(0) {}" << std::endl;
    const auto children(t.children());
    for (const auto& c : children) {
fa.stream() << "    void operator()(const " << c.name() << (c.is_primitive() ? "" : "&") << " v) const {" << std::endl;
        if (!fa.requires_hashing_helper_method(c))
fa.stream() << "        combine(test_data, v);" << std::endl;
        else
fa.stream() << "        combine(test_data, test_data_" << c.complete_identifiable_name() << "(v));" << std::endl;
fa.stream() << "    }" << std::endl;
fa.stream() << std::endl;
    }
fa.stream() << "    mutable std::size_t test_data;" << std::endl;
fa.stream() << "};" << std::endl;
fa.stream() << std::endl;
...
fa.stream() << "    return vis.test_data;" << std::endl;
fa.stream() << "}" << std::endl;
}
} } } }
