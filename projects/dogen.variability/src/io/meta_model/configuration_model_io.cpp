...
inline std::ostream& operator<<(std::ostream& s, const std::unordered_map<std::string, boost::shared_ptr<dogen::variability::meta_model::configuration> >& v) {
    s << "[";
...
std::ostream& operator<<(std::ostream& s, const configuration_model& v) {
    s << " { "
      << "\"__type__\": " << "\"dogen::variability::meta_model::configuration_model\"" << ", "
      << "\"global\": " << v.global() << ", "
      << "\"local\": " << v.local()
      << " }";
    return(s);
